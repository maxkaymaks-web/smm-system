...
| «расход», «бабки», «сколько стоило», «spend» | отвечаешь сам (см. раздел «Финансовые команды») |
...
## Если чего-то не хватает — остановись и спроси

Если для выполнения задачи нужна информация или медиа, которых нет (фото, видео, референс из прошлых сообщений, непонятный запрос) — **не ищи сам, не угадывай, не импровизируй**.

Просто напиши оператору: «Мне нужно [что именно] — пришлите, пожалуйста».

Это всегда лучше, чем потратить 5 минут на неправильный результат.

---

## Финансовые команды

Если оператор написал одну из команд ниже — **не спаунь подагента**, выполни сам:

| Сообщение оператора | Действие |
|---------------------|----------|
| `/spend` или «расход», «бабки» | отчёт за 24h |
| `/spend_week` или «расход за неделю» | отчёт за 7 дней |
| `/spend_month` или «расход за месяц» | отчёт за месяц |
| `/spend_msg` или «расход по запросам», «сколько стоил каждый запрос» | per-message, последние 10 |
| `/spend_msg20` | per-message, последние 20 |

### Как выполнить

1. Запусти нужный скрипт через Bash и захвати вывод:

```bash
# отчёт за период
node tools/spend-report.mjs --period 24h --tg
node tools/spend-report.mjs --period 7d --tg
node tools/spend-report.mjs --period month --tg

# per-message
node tools/spend-per-msg.mjs --count 10 --tg
node tools/spend-per-msg.mjs --count 20 --tg
```

2. Отправь результат в топик `finance`:

```bash
node tools/tg-send.mjs finance --text "<вывод скрипта>" --html
```

3. Ничего больше не пиши — результат уже в Telegram.
