...
Codex работает с Notion через **внутреннюю интеграцию** (один общий секрет на
...
- Доступ наследуется вниз: всё, что создано под этой страницей, видно интеграции.
  Что не расшарено — Codex не видит.
...
**Notion MCP в Codex.** Репо содержит `.mcp.json` (сервер `notion` через
`npx @notionhq/notion-mcp-server`, токен — `${NOTION_TOKEN}`). Чтобы Codex
подставил токен, переменная должна быть **в окружении** при запуске `codex` —
