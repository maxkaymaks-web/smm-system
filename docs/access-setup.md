...
**Сначала — разработчику.** Креды нужны ему первыми: без них не построить и не
протестировать интеграцию (`tools/gdrive.mjs`, подключение Notion). Передай по
защищённому каналу.

**Потом — операторам.** Когда интеграция готова, разработчик раскатывает креды на
операторов: каждый кладёт `gdrive-sa.json` в корень репозитория и прописывает значения
в свой `.env`. После этого его Claude Code работает с общими Drive-папкой и
Notion-пространством.
