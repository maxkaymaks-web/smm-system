# Как ходить в Notion (для Codex / оператора)
...
> устройства. MCP-сервер ломается, если перед запуском `codex` не
...
|---|---|---|
| Откуда берёт токен | сам читает `.env` через `loadEnv()` | из env-переменной `${NOTION_TOKEN}` на момент старта `codex` |
| Прокси | сам удаляет `*_PROXY` (tinyproxy режет Notion по IP-allow-листу) | не трогает прокси |
| Надёжность | работает всегда, пока токен в `.env` | 401, если забыли `set -a; source .env` перед `codex` |
...
```
Статусы — см. таблицу в `AGENTS.md` (`черновик` → … → `опубликовано`).
...
codex        # при первом старте подтвердить project-scoped MCP-сервер notion
```
...
`config/notion.json` (ID баз), `AGENTS.md` (статусы постов, где что лежит).
