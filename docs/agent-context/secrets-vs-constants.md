...
**Как применять:** перед записью в `.env` спросить «это даёт доступ само по себе?». Если нет — это константа, в открытый конфиг. См. [target-architecture-3-windows](target-architecture-3-windows.md).
