...
- Не забывать preload-скрипт для undici-прокси: `/root/openclaw-proxy-preload.cjs` + systemd drop-in `/root/.config/systemd/user/openclaw-gateway.service.d/proxy-preload.conf` с `Environment=NODE_OPTIONS=--require=/root/openclaw-proxy-preload.cjs`. Без него Node-нативный `fetch` в image-gen и других плагинах OpenClaw игнорит `HTTPS_PROXY` → запросы к fal.ai/OpenAI/Anthropic идут напрямую с RU-IP и обрываются на TLS handshake. Скрипт ставит `EnvHttpProxyAgent` как global undici dispatcher — `fetch()` подхватывает прокси из env. В логе должна быть строка `[proxy-preload] global undici dispatcher = EnvHttpProxyAgent (HTTPS_PROXY=set)` при старте.
- Не забывать держать `NO_PROXY` синхронным в `/etc/environment` и `/root/smm-system/.env`. Обязательно включать: `localhost,127.0.0.1,::1,5.42.117.201,5.2.66.188,api.telegram.org,s3.twcstorage.ru`. `api.telegram.org` доступен из РФ напрямую — прокси добавляет latency и иногда `CONNECT tunnel failed`. `s3.twcstorage.ru` — РФ-хост, тинипрокси к нему всё равно не имеет смысла.
