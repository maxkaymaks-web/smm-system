...
- Не забывать держать `NO_PROXY` синхронным в `/etc/environment` и `/root/smm-system/.env`. Обязательно: `localhost,127.0.0.1,::1,5.42.117.201,5.2.66.188,s3.twcstorage.ru`. **Telegram (`api.telegram.org`) НЕ кладём в `NO_PROXY`** — он заблокирован из РФ и должен идти через тинипрокси, как и fal.ai/OpenAI/Anthropic. `s3.twcstorage.ru` — РФ-хост, ему прокси не нужен.
